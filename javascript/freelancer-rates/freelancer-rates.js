...
export function monthRate(ratePerHour, discount) {
  ...
  let discountApplied = 1 - discount;
  let result = (dailyRate * 22) * discountApplied;
...
export function daysInBudget(budget, ratePerHour, discount) {
  let monthRateCost = monthRate(ratePerHour, discount);
  let discountedDailyRate = monthRateCost / 22;
  let budgetCovers = budget / discountedDailyRate;

  return Math.floor(budgetCovers);
